# Openfoot Manager — Guide pour les Testeurs Beta

Salut et bienvenue dans la bêta d'**Openfoot Manager** ! Avant tout — merci énormément d'être là. Sérieusement. Le fait que tu sois prêt à jouer à un jeu inachevé et à nous aider à l'améliorer, ça compte énormément pour nous.
...
### Ce qui N'EST PAS dans cette bêta
...
Si tu veux discuter avec l'équipe ou d'autres joueurs, tu peux aussi rejoindre le serveur Discord : https://discord.gg/2CXaesaukT
...
- **Linux avec une carte graphique NVIDIA** — si la fenêtre reste blanche ou que le jeu se ferme au démarrage, lance-le avec `OFM_GPU_PROFILE=safe`. Après deux lancements ratés d'affilée, le jeu le détecte tout seul et bascule automatiquement sur le mode sûr. Voir [Graphismes sous Linux](LINUX_GRAPHICS.md)
...
Ton feedback pendant cette bêta n'est pas juste un "bonus sympa" — il façonne littéralement ce que ce jeu va devenir. Chaque bug que tu signales, chaque "ça m'a perdu", chaque "ce serait cool si..." — tout ça compte.
