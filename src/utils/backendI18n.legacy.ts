...
const LEGACY_WEEKLY_DIGEST_WEEK_LABEL_RE = /^Week of (?<weekStart>\d{4}-\d{2}-\d{2})$/;
...
function resolveLegacyDelegatedRenewalsBody(body: string, resolve: BackendTextResolver): string {
  return body
    .split("\n")
    .map((line) => {
      ...
      if (status?.groups) {
        const detail = resolveLegacyDelegatedRenewalsDetail(status.groups.detail, resolve);
        const key =
...
export function resolveLegacyDelegatedRenewalsMessage(
  ...
  if (message.subject_key || message.body_key || message.sender_key || message.sender_role_key) {
    return message;
  }
...
export function normalizeNewsParams(article: NewsArticle): Record<string, string> | undefined {
