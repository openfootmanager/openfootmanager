...
function makeHook(enabled = true) {
  ...
  const applySnapshot = vi.fn((s: S) => {
    snapshot = s;
  });
  const onDirty = vi.fn();

  const hook = renderHook(() => useUndoRedo({ getSnapshot, applySnapshot, enabled, onDirty }));
...
describe("useUndoRedo", () => {
  ...
  it("canUndo becomes true after pushHistory", () => {
    const { hook } = makeHook();
    act(() => {
      hook.result.current.pushHistory({ value: 1 });
    });
  ...
  it("undo restores the pushed snapshot", () => {
    ...
    act(() => {
      hook.result.current.pushHistory({ value: 42 });
    });
    act(() => {
  ...
  it("redo after undo restores the pre-undo snapshot", () => {
    const { hook, applySnapshot } = makeHook();
    act(() => {
      hook.result.current.pushHistory({ value: 10 });
    });
    act(() => {
      hook.result.current.handleUndo();
    });
  ...
  it("pushHistory clears the redo stack", () => {
    ...
    expect(hook.result.current.canRedo).toBe(true);
    act(() => {
      hook.result.current.pushHistory({ value: 2 });
    });
    expect(hook.result.current.canRedo).toBe(false);
  });
